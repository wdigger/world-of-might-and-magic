...
void UI_OnMouseRightClick(int mouse_x, int mouse_y) {
    ...
    switch (current_screen_type) {
        ...
        case CURRENT_SCREEN::SCREEN_CHEST: {
            if (!pPlayers[uActiveCharacter]->CanAct()) {
                ...
                if (inventoryYCoord >= 0 && inventoryYCoord < chestheight &&
                    ...
                    int chestindex = vChests[pGUIWindow_CurrentMenu->buttonId].pInventoryIndices[invMatrixIndex];
                    if (chestindex < 0) {
                        invMatrixIndex = (-(chestindex + 1));
                        chestindex = vChests[pGUIWindow_CurrentMenu->buttonId].pInventoryIndices[invMatrixIndex];
                    }

                    if (chestindex) {
                        int itemindex = chestindex - 1;

                        GameUI_DrawItemInfo(&vChests[pChestWindow->buttonId].igChestItems[itemindex]);
                    }
                }
            }
            break;
                           ...
                           // главном меню показывает всплывающее окно)
        {
            ...
            if ((int)pY > (int)pViewport->uViewportBR_Y) {
                popup_window.buttonId = ((int)pX / 118);
                if ((signed int)pX / 118 < 4) {  // portaits zone
                    ...
                    GameUI_CharacterQuickRecord_Draw(
                        &popup_window, pPlayers[popup_window.buttonId + 1]);
                }
